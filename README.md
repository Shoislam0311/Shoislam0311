<div align="center">

# <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Food/Teacup%20Without%20Handle.png" alt="Teacup Without Handle" width="35" height="35" />  Hey there, I'm Showaib
<img src="https://github.com/Shoislam0311/Shoislam0311/blob/main/tenor.gif?raw=true" width="120" height="120" style="border-radius: 50%">

</div>

Developer from Bangladesh. I build web products, APIs, and small personal projects.

<a href="https://github.com/Shoislam0311?tab=repositories">Repositories</a>
&nbsp; · &nbsp;
<a href="https://github.com/Aniraku">Aniraku organization</a>
&nbsp; · &nbsp;
<a href="mailto:sho.islam0311@gmail.com">Email</a>

</div>

---

## About

I work across frontend interfaces, backend services, data, and deployment. I like projects that solve a clear problem and are easy for someone else to understand later.

Most of the work here is either part of **Aniraku**, the organization I work on with others, or a smaller project I built myself. I would rather show the actual sites, repositories, and screens than describe myself with a long list of buzzwords.

## Aniraku

<a href="https://www.aniraku.tech/">Open the web app</a>
&nbsp; · &nbsp;
<a href="https://github.com/Aniraku/Aniraku">Web source</a>
&nbsp; · &nbsp;
<a href="https://github.com/Aniraku/Aniraku-App">Android source</a>
&nbsp; · &nbsp;
<a href="https://aniraku.github.io/Aniraku-App/">Android download site</a>

**Aniraku** is an open-source anime discovery and viewing platform. The web app connects catalog browsing, schedules, recommendations, playback, watch history, ratings, bookmarks, comments, and progress in one place.

The public web project uses `React`, `Vite`, `Artplayer`, `HLS.js`, `AniList`, and `Supabase`. The Android project has its own player, library, history, relationships, provider controls, and Android 9+ distribution path.

### Android app screens

These are screenshots from the Android project repository.

<p align="center">
  <a href="https://github.com/Aniraku/Aniraku-App"><img src="https://raw.githubusercontent.com/Aniraku/Aniraku-App/main/site/assets/screens/home.jpg" width="23%" alt="Aniraku Android Home screen" /></a>
  <a href="https://github.com/Aniraku/Aniraku-App"><img src="https://raw.githubusercontent.com/Aniraku/Aniraku-App/main/site/assets/screens/catalog.jpg" width="23%" alt="Aniraku Android Catalog screen" /></a>
  <a href="https://github.com/Aniraku/Aniraku-App"><img src="https://raw.githubusercontent.com/Aniraku/Aniraku-App/main/site/assets/screens/watch.jpg" width="23%" alt="Aniraku Android Watch screen" /></a>
  <a href="https://github.com/Aniraku/Aniraku-App"><img src="https://raw.githubusercontent.com/Aniraku/Aniraku-App/main/site/assets/screens/profile.jpg" width="23%" alt="Aniraku Android Profile screen" /></a>
</p>

## My projects

### [Sikder Villa & Resort](https://sikder-three.vercel.app/)

A resort website for a property in Kuakata, Bangladesh. It covers rooms, dining, events, wellness, gallery content, contact, and reservations.

[Live site](https://sikder-three.vercel.app/) · [Source](https://github.com/Shoislam0311/Sikder)

`Next.js` · `React` · `TypeScript` · `Tailwind CSS` · `Prisma` · `SQLite`

### [Birthday Portfolio](https://birthday-portfolio-for-my-sis.vercel.app/)

A personal birthday website made for my sister. It includes messages, photos, motion, interactive wishes, cards, dialogs, and a responsive layout.

[Live site](https://birthday-portfolio-for-my-sis.vercel.app/) · [Source](https://github.com/Shoislam0311/Birthday-Portfolio-for-My-Sis)

`React` · `TypeScript` · `Vite` · `Tailwind CSS` · `Framer Motion`

### [Miruro API V3](https://miruro-api-v3.onrender.com/)

A small Python/FastAPI service with configuration for Vercel, Railway, Render, Docker, and Replit.

[Live deployment](https://miruro-api-v3.onrender.com/) · [Source](https://github.com/Shoislam0311/miruro-api-v3)

`Python` · `FastAPI` · `Docker` · `Uvicorn`

## Tools I use

<div align="center">

<a href="https://www.typescriptlang.org/"><img src="https://cdn.simpleicons.org/typescript/3178C6" width="32" height="32" alt="TypeScript" /></a>
<a href="https://react.dev/"><img src="https://cdn.simpleicons.org/react/61DAFB" width="32" height="32" alt="React" /></a>
<a href="https://nextjs.org/"><img src="https://cdn.simpleicons.org/nextdotjs/111111" width="32" height="32" alt="Next.js" /></a>
<a href="https://go.dev/"><img src="https://cdn.simpleicons.org/go/00ADD8" width="32" height="32" alt="Go" /></a>
<a href="https://www.python.org/"><img src="https://cdn.simpleicons.org/python/3776AB" width="32" height="32" alt="Python" /></a>
<a href="https://git-scm.com/"><img src="https://cdn.simpleicons.org/git/F05032" width="32" height="32" alt="Git" /></a>
<a href="https://www.prisma.io/"><img src="https://cdn.simpleicons.org/prisma/111111" width="32" height="32" alt="Prisma" /></a>

</div>

📊 GitHub Analytics

<div align="center">
  
<img width="49%" src="https://github-readme-stats.vercel.app/api?username=danish-mar&show_icons=true&theme=midnight-purple&hide_border=true&bg_color=0d1117&title_color=a855f7&icon_color=a855f7&text_color=c9d1d9&count_private=true&include_all_commits=true" />
<img width="49%" src="https://github-readme-streak-stats.herokuapp.com/?user=danish-mar&theme=midnight-purple&hide_border=true&background=0d1117&stroke=a855f7&ring=a855f7&fire=a855f7&currStreakLabel=a855f7" />

<img width="49%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=danish-mar&layout=compact&theme=midnight-purple&hide_border=true&bg_color=0d1117&title_color=a855f7&text_color=c9d1d9&langs_count=8" />
<img width="49%" src="https://github-readme-activity-graph.vercel.app/graph?username=danish-mar&theme=tokyo-night&hide_border=true&bg_color=0d1117&color=a855f7&line=a855f7&point=c9d1d9" />

</div>

<div align="center">
  
![](https://github-profile-trophy.vercel.app/?username=Shoislam0311&theme=dracula&no-frame=true&no-bg=true&margin-w=4&column=7)

</div>
## Contact

If you want to talk about one of these projects, open an issue in the relevant repository or send me an email at [sho.islam0311@gmail.com](mailto:sho.islam0311@gmail.com).

<div align="center"><sub>Showaib Islam · GitHub profile</sub></div>
