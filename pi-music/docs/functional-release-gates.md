...
| **Visible playback** | A real official YouTube player is present in the native app and its ready/cue/play/pause/end/error/autoplay-blocked events update Pi-Music’s player state. | **Implemented but not live-verified.** The native screen creates an official visible IFrame player only for a selected returned video, preserves the provider controls, and maps ready, state, error, and autoplay-blocked events. Lifecycle mapping is unit-verified and the Tauri process starts. A real installed-app event record remains required. |
...
| **Cross-platform release** | Linux, Windows, and macOS artifacts are built; each has an installation and core-flow smoke record. | **CI artifact builds verified.** GitHub Actions completed the current source-validation job and non-release Linux, Windows, and macOS desktop package jobs. No operating-system installation or connected-account core-flow smoke record exists yet, so this is not release evidence. |

> **Linux package-inspection note:** The latest non-release Linux artifact is present in GitHub Actions metadata (approximately 208 MB and not expired). A sandbox retrieval attempt timed out before the archive could be downloaded, so its package contents and installation behavior remain unverified. This limitation does not change the required installation/core-flow gate.

## Regression evidence

The current native Rust regression suite completed successfully with eight tests covering PKCE/state construction, callback parsing, token refresh behavior, encrypted-vault fresh-instance recovery, native runtime policy, bounded discovery input, and listener-safe provider failure translation. Frontend Vitest coverage and the production Vite build also passed for the current source. These are source-level checks, not substitutes for the installed-app gates above.

Temporary native OAuth and count-only data-validation binaries have been removed from the source tree after their evidence was recorded. The production app retains only its native OAuth, encrypted-vault, and authorized YouTube data paths.
